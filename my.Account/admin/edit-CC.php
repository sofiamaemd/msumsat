          ...
          <div class="no-1-con con-fix">
            <div class="col-1-row-1">
              <?php
                ...
                if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submitCC'])) {
                    // Retrieve form data
                    $CCTitle = $_POST['CC_title'];
                    $CCFileName = $_FILES['CC_file_name']['name'];
                    $oldFileName = $_POST['old_CC_file_name'];

                    // Perform the update operation for CC
                    $stmt = $pdo->prepare("UPDATE citizen_charters SET CC_title = ?, CC_file_name = ? WHERE CC_file_name = ?");
                    $stmt->execute([$CCTitle, $CCFileName, $oldFileName]);

                    // Move uploaded file to the desired location for CC
                    ...
                    $previousFilePath = '../../home/citizens-charter/' . $oldFileName;
                    if (file_exists($previousFilePath) && $CCFileName !== $oldFileName) {
                        unlink($previousFilePath);
                    }

                    // Redirect the user to a success page for CC
                    header("Location: transparency-seal.php?updateCC=successful!");
                    exit();
                }

                // Check if the delete action is triggered
                if (isset($_GET['action']) && $_GET['action'] === 'deleteCC') {
                    $fileToDelete = urldecode($_GET['file']); // Decode the file name

                    // Delete the file from the server
                    $filePath = '../../home/citizens-charter/' . $fileToDelete;
                    if (file_exists($filePath)) {
                        unlink($filePath);

                        // Delete the corresponding record from the database
                        $stmt = $pdo->prepare("DELETE FROM citizen_charters WHERE CC_file_name = ?");
                        $stmt->execute([$fileToDelete]);
                    }
                }

                // Prepare and execute the SQL query to fetch the data
                $stmt = $pdo->prepare("SELECT CC_title, CC_file_name FROM citizen_charters");
                $stmt->execute();
                $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
              ?>
              <?php
                // Check if the CC file is requested for editing
                if (isset($_GET['file'])) {
                    $fileToEdit = $_GET['file'];

                    // Fetch the corresponding record from the database
                    $stmt = $pdo->prepare("SELECT CC_title, CC_file_name FROM citizen_charters WHERE CC_file_name LIKE ?");
                    ...
                    if ($result) {
                        ...
                        $oldFileName = $result['CC_file_name'];
              ?>
              <div class="uploader">
                  <h1 style="text-align:center">Edit Citizen's Charter</h1><br>
                  <form action="" method="POST" enctype="multipart/form-data">
                      <input type="hidden" name="old_CC_file_name" value="<?php echo $oldFileName; ?>">
                      <input type="text" name="CC_title" placeholder="Description" value="<?php echo $result['CC_title']; ?>" required>
                      <input type="file" name="CC_file_name" accept=".pdf" required>
                      <input type="submit" name="submitCC" value="Update">
                  </form>
              </div>
              <?php
                  } else {
