              ...
              <div class="first_half_con">
                <?php
                  ...
                  if (isset($_POST['directory_submit'])) {
                      ...
                      if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
                          ...
                          $imageTargetFile = 'staff-directory/' . basename($imageFile);
                          move_uploaded_file($imageTmp, $imageTargetFile);
                      } else {
                          $imageTargetFile = '';
                      }
                      // Insert data into the database
                      $insertQuery = "INSERT INTO staff_directory (fullname, position, title, comment, image) VALUES (?, ?, ?, ?, ?)";
                      $stmt = $mysqli->prepare($insertQuery);
                      $stmt->bind_param("sssss", $fullname, $position, $title, $comment, $imageTargetFile);
                      $stmt->execute();
                      if ($stmt->affected_rows > 0) {
                          echo "Data inserted successfully.";
                          header("Location:staff-directory.php?action=successful"); // Redirect to the specified page
                      } else {
                ...
                <?php
                  ...
                  if (isset($_GET['search'])) {
                      ...
                      $query = "SELECT id, fullname, position, title, comment, image FROM staff_directory WHERE fullname LIKE '%$searchQuery%' ORDER BY id DESC";
                      $result = $mysqli->query($query);
                  } else {
                      // Retrieve all employee information from the database
                      $result = $mysqli->query("SELECT id, fullname, position, title, comment, image FROM staff_directory ORDER BY id DESC");
                  }
                ?>
              </div>
                        ...
                        if ($result && $result->num_rows > 0) {
                          ...
                          echo '<div class="collapse-con">';
                           while ($row = $result->fetch_assoc()) {
                            ...
                            echo '<form class="employee-form" action="delete-sd.php" method="POST">';
