                ...
                <div class="first_half_con">
                  <?php
                    ...
                    if (isset($_POST['directory_submit'])) {
                        ...
                        if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
                            ...
                            $imageTargetFile = 'msu-system-officials/' . basename($imageFile);
                        ...
                        $insertQuery = "INSERT INTO msu_system_officials (fullname, position, title, comment, image) VALUES (?, ?, ?, ?, ?)";
                        ...
                        if ($stmt->affected_rows > 0) {
                            ...
                            header("Location:msu-system-officials.php?action=successful"); // Redirect to the specified page
                        } else {
                  ...
                  <?php
                    ...
                    if (isset($_GET['search'])) {
                        ...
                        $query = "SELECT id, fullname, position, title, comment, image FROM msu_system_officials WHERE fullname LIKE '%$searchQuery%' ORDER BY id DESC";
                        $result = $mysqli->query($query);
                    } else {
                        // Retrieve all employee information from the database
                        $result = $mysqli->query("SELECT id, fullname, position, title, comment, image FROM msu_system_officials ORDER BY id DESC");
                    }
                  ?>
                </div>
                        ...
                        // Check if any employee information found
                          if ($result && $result->num_rows > 0) {
                            ...
                            while ($row = $result->fetch_assoc()) {
                              ...
                              echo '<form class="employee-form" action="delete-mso.php" method="POST">';
