          ...
          <div class="no-1-con con-fix">
            <div class="col-1-row-1">
                <?php
                    ...
                    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submitABT'])) {
                        // Retrieve form data
                        $ABTTitle = $_POST['ABT_title'];
                        $ABTFileName = $_FILES['ABT_file_name']['name'];
                        $oldFileName = $_POST['old_ABT_file_name'];

                        // Perform the update operation for ABT
                        $stmt = $pdo->prepare("UPDATE approved_budget_targets SET ABT_title = ?, ABT_file_name = ? WHERE ABT_file_name = ?");
                        $stmt->execute([$ABTTitle, $ABTFileName, $oldFileName]);

                        // Move uploaded file to the desired location for ABT
                        ...
                        $previousFilePath = 'approved-budget-targets/' . $oldFileName;
                        if (file_exists($previousFilePath) && $ABTFileName !== $oldFileName) {
                            unlink($previousFilePath);
                        }

                        // Redirect the user to a success page for ABT
                        header("Location: transparency-seal.php?updateABT=successful!");
                        exit();
                    }
                    ...
                    $stmt = $pdo->prepare("SELECT ABT_title, ABT_file_name FROM approved_budget_targets");
                    $stmt->execute();
                    $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
                ?>
                <?php
                    // Check if the ABT file is requested for editing
                    if (isset($_GET['file'])) {
                        $fileToEdit = $_GET['file'];

                        // Fetch the corresponding record from the database
                        $stmt = $pdo->prepare("SELECT ABT_title, ABT_file_name FROM approved_budget_targets WHERE ABT_file_name LIKE ?");
                        ...
                        if ($result) {
                            ...
                            $oldFileName = $result['ABT_file_name'];
                ?>
                <div class="uploader">
                    <h1 style="text-align:center">II. DBM Approved Budget and Targets Edit</h1><br>
                    <form action="" method="POST" enctype="multipart/form-data">
                        <input type="hidden" name="old_ABT_file_name" value="<?php echo $oldFileName; ?>">
                        <input type="text" name="ABT_title" placeholder="Description" value="<?php echo $result['ABT_title']; ?>" required>
                        <input type="file" name="ABT_file_name" accept=".pdf" required>
                        <input type="submit" name="submitABT" value="Update">
                    </form>
                </div>
                <?php
                    } else {
