          ...
          <div class="no-1-con con-fix">
            <div class="col-1-row-1">
                <?php
                    ...
                    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submitAFR'])) {
                        // Retrieve form data
                        $AFRTitle = $_POST['AFR_title'];
                        $AFRFileName = $_FILES['AFR_file_name']['name'];
                        $oldFileName = $_POST['old_AFR_file_name'];

                        // Perform the update operation for AFR
                        $stmt = $pdo->prepare("UPDATE annual_financial_reports SET AFR_title = ?, AFR_file_name = ? WHERE AFR_file_name = ?");
                        $stmt->execute([$AFRTitle, $AFRFileName, $oldFileName]);

                        // Move uploaded file to the desired location for AFR
                        $targetDirectoryAFR = "nnual-financial-reports/";
                        ...
                        $previousFilePath = 'nnual-financial-reports/' . $oldFileName;
                        if (file_exists($previousFilePath) && $AFRFileName !== $oldFileName) {
                            unlink($previousFilePath);
                        }

                        // Redirect the user to a success page for AFR
                        header("Location: transparency-seal.php?updateAFR=successful!");
                        exit();
                    }
                    ...
                    $stmt = $pdo->prepare("SELECT AFR_title, AFR_file_name FROM annual_financial_reports");
                    $stmt->execute();
                    $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
                ?>
                <?php
                    // Check if the AFR file is requested for editing
                    if (isset($_GET['file'])) {
                        $fileToEdit = $_GET['file'];

                        // Fetch the corresponding record from the database
                        $stmt = $pdo->prepare("SELECT AFR_title, AFR_file_name FROM annual_financial_reports WHERE AFR_file_name LIKE ?");
                        ...
                        if ($result) {
                            ...
                            $oldFileName = $result['AFR_file_name'];
                ?>
                <div class="uploader">
                    <h1 style="text-align:center">VII. Off-budget Income Reports Edit</h1><br>
                    <form action="" method="POST" enctype="multipart/form-data">
                        <input type="hidden" name="old_AFR_file_name" value="<?php echo $oldFileName; ?>">
                        <input type="text" name="AFR_title" placeholder="Description" value="<?php echo $result['AFR_title']; ?>" required>
                        <input type="file" name="AFR_file_name" accept=".pdf" required>
                        <input type="submit" name="submitAFR" value="Update">
                    </form>
                </div>
                <?php
                    } else {
