                  ...
                  <li class="nav__item">
                    ...
                    <ul class="nav__drop">
                      <div class="padding">
                        ...
                        <li>
                          <a href="hrmt.php">DHRMT</a>
                        </li>
                        <li>
                          <a href="hess.php">DHESS</a>
                        </li>
                        <li>
                          <a href="it.php">DIT</a>
                        </li>
                        <li>
                          <a href="nsm.php">DNSM</a>
                        </li>
                        <li>
                          <a href="se.php">DSE</a>
                        </li>
          ...
          <div class="no-1-con con-fix">
            <div class="col-1-row-1">
                  <div class="first_half_con">
                    <?php
                      ...
                      if (isset($_POST['directory_submit'])) {
                          ...
                          if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
                              ...
                              $imageTargetFile = 'hess-directory/' . basename($imageFile);
                          ...
                          $insertQuery = "INSERT INTO hess_directory (fullname, position, title, comment, image) VALUES (?, ?, ?, ?, ?)";
                          $stmt = $mysqli->prepare($insertQuery);
                          $stmt->bind_param("sssss", $fullname, $position, $title, $comment, $imageTargetFile);
                          $stmt->execute();

                          if ($stmt->affected_rows > 0) {
                              echo "Data inserted successfully.";
                              header("Location:hess.php?action=successful"); // Redirect to the specified page
                          } else {
                    ...
                    ?>
                    <div class="uploader">
                      <h1>DHESS Faculty Directory</h1>
                        <form method="POST" enctype="multipart/form-data">
                            <div>
                              <input type="text" name="fullname" id="fullname" required placeholder="Full Name">
                            </div>
                            <div>
                              <input type="text" name="position" id="position" required placeholder="Position">
                            </div>
                            <div>
                              <input type="text" name="title" id="title" required placeholder="Overview">
                            </div>
                            <div>
                              <textarea name="comment" id="comment" required placeholder="Description"></textarea>
                            </div>
                            <div>
                    ...
                    <?php
                      ...
                      if (isset($_GET['search'])) {
                          ...
                          $query = "SELECT id, fullname, position, title, comment, image FROM hess_directory WHERE fullname LIKE '%$searchQuery%' ORDER BY id DESC";
                          $result = $mysqli->query($query);
                      } else {
                          // Retrieve all employee information from the database
                          $result = $mysqli->query("SELECT id, fullname, position, title, comment, image FROM hess_directory ORDER BY id DESC");
                      }
                    ?>
                  </div>
                          ...
                          // Check if any employee information found
                            if ($result && $result->num_rows > 0) {
                              ...
                              while ($row = $result->fetch_assoc()) {
                                ...
                                echo '<form class="employee-form" action="delete-hess.php" method="POST">';
                                ...
                                echo '<center><img src="' . $row['image'] . '" alt=""></center>';
                                echo '</div>';
                                echo '</div>';
                                echo '<div class="collapse-1-whole">';
