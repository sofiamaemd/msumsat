                  ...
                  <div class="first_half_con">
                    <?php
                      ...
                      if (isset($_POST['directory_submit'])) {
                          ...
                          if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
                              ...
                              $imageTargetFile = 'cs-directory/' . basename($imageFile);
                          ...
                          $insertQuery = "INSERT INTO cs_directory (fullname, position, title, comment, image) VALUES (?, ?, ?, ?, ?)";
                          ...
                          if ($stmt->affected_rows > 0) {
                              ...
                              header("Location:cs.php?action=successful"); // Redirect to the specified page
                          } else {
                    ...
                    <?php
                      ...
                      if (isset($_GET['search'])) {
                          ...
                          $query = "SELECT id, fullname, position, title, comment, image FROM cs_directory WHERE fullname LIKE '%$searchQuery%' ORDER BY id DESC";
                          $result = $mysqli->query($query);
                      } else {
                          // Retrieve all employee information from the database
                          $result = $mysqli->query("SELECT id, fullname, position, title, comment, image FROM cs_directory ORDER BY id DESC");
                      }
                    ?>
                  </div>
                          ...
                          // Check if any employee information found
                            if ($result && $result->num_rows > 0) {
                              ...
                              while ($row = $result->fetch_assoc()) {
                                ...
                                echo '<form class="employee-form" action="delete-cs.php" method="POST">';
