          ...
          <div class="no-1-con con-fix">
            <div class="col-1-row-1">
                  <div class="first_half_con">
                    <?php
                      ...
                      if (isset($_POST['directory_submit'])) {
                          ...
                          if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
                              ...
                              $imageTargetFile = 'employees/' . basename($imageFile);
                              move_uploaded_file($imageTmp, $imageTargetFile);
                          } else {
                              $imageTargetFile = '';
                          }
                          // Insert data into the database
                          $insertQuery = "INSERT INTO employees (fullname, position, title, comment, image) VALUES (?, ?, ?, ?, ?)";
                          ...
                          if ($stmt->affected_rows > 0) {
                              ...
                              header("Location:campus-officials-directory.php?action=successful"); // Redirect to the specified page
                          } else {
                    ...
                    ?>
                    <div class="uploader">
                      <h1>Campus Officials Directory</h1>
                    ...
                    <?php
                      ...
                      if (isset($_GET['search'])) {
                          ...
                          $query = "SELECT id, fullname, position, title, comment, image FROM employees WHERE fullname LIKE '%$searchQuery%' ORDER BY id DESC";
                          $result = $mysqli->query($query);
                      } else {
                          // Retrieve all employee information from the database
                          $result = $mysqli->query("SELECT id, fullname, position, title, comment, image FROM employees ORDER BY id DESC");
                      }
                    ?>
                  </div>
                          ...
                          // Check if any employee information found
                            if ($result && $result->num_rows > 0) {
                              ...
                              while ($row = $result->fetch_assoc()) {
                                ...
                                echo '<form class="employee-form" action="delete-employee.php" method="POST">';
